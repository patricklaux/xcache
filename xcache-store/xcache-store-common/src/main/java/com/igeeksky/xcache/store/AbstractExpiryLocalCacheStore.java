 ...
 */
public abstract class AbstractExpiryLocalCacheStore<K, V> extends AbstractCacheStore<K, V> {
    ...
    public AbstractExpiryLocalCacheStore(String name, CacheProperties.Local local, Class<K> keyType, Class<V> valueType,
    ...
    protected abstract ExpiryCacheValue<Object> doStoreGet(K key);

    protected abstract void doStorePut(K key, ExpiryCacheValue<Object> cacheValue);
    ...
    private ExpiryCacheValue<V> fromStoreValue(ExpiryCacheValue<Object> cacheValue) {
        if (null == cacheValue) {
            return null;
        }
        ...
        if (null == storeValue) {
            return (ExpiryCacheValue<V>) cacheValue;
        }
        ...
        if (enableSerializeValue) {
            ...
            return CacheValues.newExpiryCacheValue(valueSerializer.deserialize(value), cacheValue.getExpiryTime());
        }
     ...
     */
    private ExpiryCacheValue<Object> toStoreValue(V value) {
        if (null == value) {
            return CacheValues.emptyExpiryCacheValue();
        }
        ...
        if (enableSerializeValue) {
            ...
            return CacheValues.newExpiryCacheValue(storeValue);
        }
        return CacheValues.newExpiryCacheValue(value);
    }
